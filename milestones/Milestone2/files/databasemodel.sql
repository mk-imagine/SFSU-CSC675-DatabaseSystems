...
CREATE TABLE IF NOT EXISTS `userShopList` (
  ...
  `is_shoppingcart` BOOLEAN NOT NULL,
...
CREATE TABLE IF NOT EXISTS `orderDetails` (
  ...
  CONSTRAINT `ORDERDETAILS_SHOPLIST_FK`
    FOREIGN KEY (`shoplist`)
    REFERENCES `shopList` (`shoplist_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `receipt` (
  ...
  CONSTRAINT `RECEIPT_ORDERDETAILS_FK`
    FOREIGN KEY (`orderDetails`)
...
CREATE TABLE IF NOT EXISTS `paymentInfo` (
  ...
  PRIMARY KEY (`orderDetails`, `paymentMethod`),
  INDEX `PI_PAYMENTMETHOD_PK_FK_idx` (`paymentMethod` ASC) VISIBLE,
  CONSTRAINT `PI_ORDERDETAILS_PK_FK`
    FOREIGN KEY (`orderDetails`)
    REFERENCES `orderDetails` (`order_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `orderFulfillment` (
  `orderDetails` INT(10) NOT NULL,
  `fulfillmentAction` INT(20) NOT NULL,
  PRIMARY KEY (`orderDetails`, `fulfillmentAction`),
  INDEX `FULFILLMENTACTION_FK_PK_idx` (`fulfillmentAction` ASC) VISIBLE,
  CONSTRAINT `OF_ORDERDETAILS_PK_FK`
    FOREIGN KEY (`orderDetails`)
    REFERENCES `orderDetails` (`shoplist`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `OF_FULFILLMENTACTION_PK_FK`
